...
#scene = moveit_commander.PlanningSceneInterface("/tb3_4/robot_description")
group_arm = "arm"
group_name = "gripper"
group = moveit_commander.MoveGroupCommander(group_name)
grouparm = moveit_commander.MoveGroupCommander(group_arm)
...
def drive_callback(data, args):
    ...
    if iterate < 0:
        ...
        #make sure TurtleBot stops by sending a default Twist()
        pub_vel.publish(Twist())
        print('final data.x', data.x)
        print('Final Ball_x', ball_x)
        print('final norm_x', norm_x)
        print('final norm_y', norm_y)
...
def find(x,y):
    ...
    rospy.init_node('moveit_routine_fix', anonymous=True)
    img_sub = rospy.Subscriber("/coordinates",Point, drive_callback, (x, y))
    rospy.spin()
